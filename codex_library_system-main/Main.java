...
public class Main {
    ...
    private static void DeleteMaterial() {
        ...
        String deleteID = input.nextLine().trim(); // Trim whitespace from the input
    
        // Check if the Material ID exists in the library
        boolean exists = library.stream().anyMatch(mat -> mat.getMaterialID().equals(deleteID));
        if (!exists) {
            System.out.println("Error: Material not found!");
            return;
        }
    
        // Remove the material with the matching ID
